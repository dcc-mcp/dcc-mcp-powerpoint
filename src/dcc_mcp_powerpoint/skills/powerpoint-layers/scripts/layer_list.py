"""powerpoint-layers / layer_list — inventory the semantic layers of a PPTX
...
from dcc_mcp_powerpoint.layers import list_layers
...
def run(params: dict) -> None:
    ...
    if loaded is None:
        return
    prs, path = loaded
    report = list_layers(prs, str(path))
    print(json.dumps({"success": True, "message": f"{len(report['layers'])} layers across {report['slide_count']} slides", "context": report}, ensure_ascii=False))
    ...
    if not sys.stdin.isatty():
        ...
        if raw.strip():
            try:
                ...
                params = {}
    if not params:
        parser = argparse.ArgumentParser(description="inventory the semantic layers of a PPTX")
