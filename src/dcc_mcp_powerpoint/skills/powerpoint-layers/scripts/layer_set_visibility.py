"""powerpoint-layers / layer_set_visibility — show or hide a semantic layer across slides
...
from dcc_mcp_powerpoint.layers import set_layer_visibility
...
def run(params: dict) -> None:
    ...
    if loaded is None:
        return
    prs, path = loaded
    report = set_layer_visibility(prs, str(params["layer"]), _as_bool(params["visible"]), _slides(params))
    target = _save(prs, path, params)
    print(json.dumps({"success": True, "message": f"layer '{params['layer']}' visible={params['visible']} ({report['changed']} shapes changed)", "context": {"output": target, "visibility": report}}, ensure_ascii=False))
    ...
    if not sys.stdin.isatty():
        ...
        if raw.strip():
            try:
                ...
                params = {}
    if not params:
        parser = argparse.ArgumentParser(description="show or hide a semantic layer across slides")
        ...
        parser.add_argument("--visible", dest="visible", required=True, help="true=show false=hide")
