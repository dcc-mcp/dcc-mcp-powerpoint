"""powerpoint-layers / layer_reorder — restack a layer in z-order
...
from dcc_mcp_powerpoint.layers import reorder_layer
...
def run(params: dict) -> None:
    ...
    if loaded is None:
        return
    prs, path = loaded
    report = reorder_layer(prs, str(params["layer"]), str(params["position"]), _slides(params))
    target = _save(prs, path, params)
    print(json.dumps({"success": True, "message": f"layer '{params['layer']}' restacked to '{params['position']}' on {report['slides_moved']} slides", "context": {"output": target, "reorder": report}}, ensure_ascii=False))
    ...
    if not sys.stdin.isatty():
        ...
        if raw.strip():
            try:
                ...
                params = {}
    if not params:
        parser = argparse.ArgumentParser(description="restack a layer in z-order")
        ...
        parser.add_argument("--position", dest="position", required=True, help="front | back | above LAYER | below LAYER")
